...
def decoupage(
    chemin
):  # Découpe l'image en plein de portion plus petites pour les analyser séparement
    ...
    L = [i / 5 for i in range(1, 5 + 1)
         ]  # Liste des tailles relatives des sous-images

    for k in range(len(L)):
        print("Etape " + str(k + 1) + "/" +
              str(len(L)))  # Affiche l'avancement
        rapport = L[
        ...
        for dx in range(0, l - taille, step):
            for dy in range(0, h - taille, step):
                nvNpArray = np.array([[[img[y][x][i] for i in range(d)]
                                       for x in range(dx, dx + taille)]
                                      for y in range(dy, dy + taille)])
                nvImage = Image.fromarray(nvNpArray)
                nvImage.save("/tmp/images/rapport" + str(rapport) + "_x" +
                             str(dx) + "_y" + str(dy) + ".jpg")


def analyseDansDossier(chemin, graphPath, input_layer, output_layer,
                       label_file):
    files = [
        f for f in listdir(chemin)
        if (isfile(join(chemin, f)) and f.endswith(".jpg"))
    ]
    L = []
    ...
    for i in range(len(files)):
        ...
        file = files[i]
        (label, result) = analyseImage(graph, join(chemin, file), input_layer,
                                       output_layer, label_file)
        if (result >= 0.8):
