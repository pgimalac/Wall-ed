...
def analyseSousImage(graph, tensor, input_layer="Placeholder", output_layer="final_result", label_file="/home/pact32/ImageNet/output_labels.txt"):
...
def analyseImage(image_path = '../../test.JPG'):
    result_to_return = []
    result = subprocess.run(['./darknet_modified', 'detector', 'test', 'cfg/obj.data', 'cfg/tiny-yolo.cfg', 'backup/tiny-yolo_last.weights', image_path], stdout=subprocess.PIPE).stdout.decode('utf-8')
    result = [[int(y.split(",")[0]) for y in x.split("=")[1:]] for x in result.split("\n") if "Bounding" in x]
    ...
    for (left, top, right, bottom) in result:
        ...
        result_to_return.append([(left, top, right, bottom), analyseSousImage(load_graph(), tensor)[0]])
