...
public class Main {
    ...
    public static void getStats(int sessionID) { String query = ""; }

    public static String executePythonScriptForAI(String image)
        throws IOException {
        // image : nom de l'image

        String pythonScriptPath =
            "/home/adrien/Documents/pact/pact32/modules/COMCS/AI.py"; // à
                                                                      // définir
    ...
    public static RobotClientProcessor inform(Activite act, Eleve[] eleves)
        ...
        System.out.println(
            "[MAIN] robot connected, finishing activity creation");
    ...
    public static Eleve[] getElevesInBDD() {
        ...
        for (int i = 1; i < lastID + 1; i++) {
            String query = "SELECT * FROM ELEVES WHERE eleveID = '" +
                           Integer.toString(i) + "'";
            Eleve temp =
                new Eleve(i, Connect_bdd.lastExecuteSQL(query, "prenom"),
                          Connect_bdd.lastExecuteSQL(query, "nom"));
            liste[i - 1] = temp;
        }
        return liste;
    }
