...
def mise_a_jour_bdd():
    ...
    if int(cursor.fetchone()[0]) != int(last_id):
        cursor.execute("""SELECT * FROM GLOBALE WHERE sessionID > %s""",
                       (last_id, ))
        rows = cursor.fetchall()
        for row in rows:
            activite = Activite.objects.create(sessionID=row[0],
                                               date=row[1],
                                               heureDebut=row[2],
                                               heureFin=row[3],
                                               nbEleves=row[4])
    ...
    cursor.execute("""SELECT * FROM ELEVES WHERE eleveID > %s""",
                   (last_eleve, ))
    rows = cursor.fetchall()
    for row in rows:
...
def mise_a_jour_activite(activite, sessionID):
    ...
    if len(dechets) == 0:
        ...
        for i in range(1, len(row)):
            ...
            dechet = Dechet.objects.create(dechetID=row[i][0],
                                           activite=activite,
                                           eleve=eleve,
                                           type=row[i][3],
                                           typePropose=row[i][4],
                                           reponseEleve=row[i][5],
                                           heureRamassage=row[i][6])
    conn.close()
