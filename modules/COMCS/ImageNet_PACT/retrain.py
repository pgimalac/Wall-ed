...
def get_image_path(image_lists, label_name, index, image_dir, category):
  ...
  """
    ...
    if not category_list:
        logging.fatal('Label %s has no images in the category %s.', label_name,
                      category)
...
def run_final_eval(train_session, module_spec, class_count, image_lists,
  ...
  """
    ...
    logging.info('Final test accuracy = %.1f%% (N=%d)', test_accuracy * 100,
...
def logging_level_verbosity(logging_verbosity):
  ...
  """
    ...
    try:
        ...
        raise RuntimeError(
            'Not supported logs verbosity (%s). Use one of %s.' %
...
def main(_):
    ...
    if class_count == 1:
        logging.error(
            'Only one valid folder of images found at %s '
