...
IMG_DIR = 'Garbage classification'  # This directory stores subdirectories that are image class titles, like 'cat'
...
def build_tl_model(original_model):
    ...
    if args.verbose:
        print('Model summary before final layer addition:',
              new_model.summary())

    NUM_CLASSES = len(
        os.listdir(IMG_DIR))  # How many image classes are in our new data?
    BOTTLENECK_DIM = original_model.get_layer(index=-2).output.shape.dims[
        1]  # The number of nodes in the second to last layer of the pre-trained model.
...
def process_data():
    ...
    for i, subdir in enumerate(os.listdir(IMG_DIR)):
        for file_name in os.listdir(IMG_DIR + '/' + subdir):
            processed_imgs_list.append(
                read_file(BASE_DIR + '/' + IMG_DIR + '/' + subdir + '/' +
                          file_name))
            labels.append(i)

    ttl_num_new_imgs = len(processed_imgs_list)
    processed_imgs_array = np.asarray(processed_imgs_list).reshape(
        ttl_num_new_imgs, 224, 224, 3)
...
if __name__ == '__main__':
    ...
    if args.verbose:
        print(new_model.predict(processed_imgs_list[0].reshape(
            1, 224, 224,
            3)))  # Newly trained model prediction. [0,1] = [cat, dog].
        print(
            INCEPTIONV3.predict(processed_imgs_list[0].reshape(
                1, 224, 224,
                3)).argmax(-1))  # Original Inception model prediction.
